...
export class AppointmentListService {
  ...
  getAppoinmentListWalkin(type: string, page: number, date: string, appointmentStatus: string) {
    var url = type == null ? `${environment.apiUrl}merchantTodayAppointments?pagination=true&page=${page}&queryOrderType=FUTURE&status=${appointmentStatus}` : `${environment.apiUrl}merchantTodayAppointments?pagination=true&page=${page}&type=${type}&queryOrderType=FUTURE&status=${appointmentStatus}`;
