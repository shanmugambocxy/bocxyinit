...
export class Tab3Servcie {
  ...
  GetStylistSlots(stylistAccountId: number, date: string) {
    ...
    return this.http.get<{ data: any, status: string }>(`${environment.apiUrl}appointmentStylistSlots/${stylistAccountId}/${date}`, httpOptions).
      pipe(
        tap(_ => console.log("Appoinment style slot")),
        catchError(this.eh.handleHttpError<{ data: any, status: string }>('Appoinment style slot', { data: [], status: "Failure" }))
      );
  }
  ...
  BookAppointment(data: AppointmentBooking) {
    return this.http.post<{ data: any; status: string }>(`${environment.apiUrl}appointment`, data, httpOptions)
      .pipe(
        tap(_ => console.log('Appointment booked')),
